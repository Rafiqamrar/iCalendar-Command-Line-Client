...
public class EventFilters
{
                ...
                LocalDate toDate)
  {
    return switch (option)
    {
            ...
            case RANGE -> filterByRange(events, fromDate, toDate);
        };
    }
    
    private static List<Event> filterByDate(List<Event> events, LocalDate targetDate) {
    ...
    private static List<Event> filterByWeek(List<Event> events, LocalDate today) {
        LocalDate startOfWeek = today.with(DayOfWeek.MONDAY);
        LocalDate endOfWeek = today.with(DayOfWeek.SUNDAY);
        return filterByRange(events, startOfWeek, endOfWeek);
    }
    
    private static List<Event> filterByRange(List<Event> events, LocalDate from, LocalDate to) {
        return events.stream()
            .filter(event -> {
                LocalDate eventDate = getEventDate(event);
