...
public class Decoder {
  ...
  public static Todo makeTodo(Map<String, String> map) {
    ...
    return new Todo(UID, SUMMARY, LOCATION, STATUS, PERCENT_COMPLETE, COMPLETED, DUE, DTSTART,
        CLASS, PRIORITY, LAST_MODIFIED, DTSTAMP, SEQUENCE, ORGANIZER_name, ORGANIZER_mail);
  }
  ...
  public static Event makeEvent(Map<String, String> map) {
    ...
    Number SEQUENCE = Utils.StoNum(map.get("SEQUENCE"));
    return new Event(UID, SUMMARY, LOCATION, LAST_MODIFIED, DTSTAMP, DTSTART, DTEND, CREATED,
  ...
  public static Calendar calendar(List<Map<String, String>> maps) {
    ...
    List<CalElement> els = maps.subList(1, maps.size()).stream().map(Decoder::calElement).toList();
